...
void receivedCallback( uint32_t from, String &msg ) {
  ...
  // Parse message format: "height1,height2"
  int commaIndex = msg.indexOf(',');
  if (commaIndex != -1) {
    String height1Str = msg.substring(0, commaIndex); // Extract first value
    int newHeight = height1Str.toInt(); // Convert to integer
    
    // Validate height is within limits
    if (newHeight >= HEIGHT_MIN && newHeight <= HEIGHT_MAX) {
      height = newHeight; // Update global height variable
      Serial.printf("Target height set to: %d (%.1f mm)\n", height, height / 10.0);
      moveMotor(height); // Move motor to target height
    } else {
      Serial.printf("ERROR: Height %d out of range [%d-%d]\n", newHeight, HEIGHT_MIN, HEIGHT_MAX);
    }
  }
}
