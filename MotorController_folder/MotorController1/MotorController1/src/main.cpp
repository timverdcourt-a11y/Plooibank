...
#define MOTOR_CONTROLLER_MESSAGE_TAG "TargetHeightMotor1"
...
#define ALM_PIN 32
// GPIO pin that pulses HIGH for 200 ms to clear a latched alarm on the ESS17-04.
...
// Timestamp in ms of the most recent ActualHeightMotor1 broadcast.
...
// millis() of the last "AlarmMotor1:1" broadcast.
...
// Store a new target position when a valid TargetHeightMotor1 message is received.
...
static void handle_mesh_message(uint32_t from, const String &msg)
{
    ...
    if (msg == "CalibrateMotor1") { // Respond to a calibration command from the GUI.
        start_calibration();
        return;
    }

    if (msg == "ResetAlarmMotor1") { // Operator acknowledged the alarm — re-enable movement and clear the driver latch.
        alarm_blocked             = false;
...
void setup()
{
    ...
    pinMode(ALM_CLEAR_PIN, OUTPUT);    // Alarm-clear output: pulse HIGH for 200 ms to release the latched fault on the ESS17-04.
    ...
    prefs.begin("mc1", false); // Open the NVS namespace for this controller.
    ...
    Serial.println("MotorController1 ready for TargetHeightMotor1 messages");
}
...
void loop()
{
    ...
    if (calibrating && (limit_switch_triggered || digitalRead(LIMIT_SWITCH_PIN) == LOW)) {
        ...
        last_save_ms = millis();
        mesh.sendBroadcast("CalibratedMotor1"); // Tell the GUI that calibration is done and the height is 86.0 mm.
    }
    ...
    // End the alarm-clear pulse after 200 ms.
    ...
    if (alm_low && !alarm_active) {
        ...
        mesh.sendBroadcast("AlarmMotor1:1");
    } else if (!alm_low && alarm_active) {
        alarm_active  = false;
        alarm_blocked = false;
        mesh.sendBroadcast("AlarmMotor1:0");
    }
    ...
    if (now_ms - last_broadcast_ms >= 100) { // Broadcast the actual height to the GUI every 100 ms.
        const int actual_tenths_mm = pulses_to_tenths_mm(current_position_pulses); // Convert the current pulse count back to tenths of mm.
        mesh.sendBroadcast("ActualHeightMotor1:" + String(actual_tenths_mm)); // Send the current height so the GUI label stays up to date.
        last_broadcast_ms = now_ms; // Record the broadcast time to enforce the 100 ms interval.
    }
}
